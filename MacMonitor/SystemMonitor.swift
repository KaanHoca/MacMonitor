...
struct ProcessEntry: Identifiable {
    let id = UUID()
    let name: String
    let value: Double  // MB (ram) veya % (cpu)
}
...
class SystemMonitor: ObservableObject {
    ...
    // MARK: - Proses adı gruplama
    private func groupName(for path: String) -> String {
        // Bilinen proses grupları
    ...
    private func runPS(args: [String], completion: @escaping ([String]) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            task.arguments = args
            // Türkçe locale'de ondalık ayırıcı virgül oluyor, nokta zorla
    ...
    private func readSMCFloat(_ key: String) -> Double? {
        ...
        let kr1 = IOConnectCallStructMethod(smcConn, 2, &inData, MemoryLayout<SMCKeyData>.size, &outData, &outSize)
        guard kr1 == kIOReturnSuccess, outData.keyInfo.dataSize == 4 else { return nil }

        // Değeri oku
        var inRead = SMCKeyData()
        var outRead = SMCKeyData()
        inRead.key = fourCC(key)
        inRead.keyInfo.dataSize = 4
        inRead.data8 = 5 // kSMCReadKey
        outSize = MemoryLayout<SMCKeyData>.size
        let kr2 = IOConnectCallStructMethod(smcConn, 2, &inRead, MemoryLayout<SMCKeyData>.size, &outRead, &outSize)
    ...
    private func getTemp() -> Double {
        // Tüm çekirdeklerin ortalamasını al
        ...
        // Exponential moving average — ani sıçramaları yumuşat
        if smoothedTemp == 0 {
            smoothedTemp = avg
        } else {
